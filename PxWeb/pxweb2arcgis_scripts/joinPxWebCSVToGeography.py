...
in_pxw_csv = arcpy.GetParameterAsText(0)
in_pxw_csv_path = Path(in_pxw_csv)
...
in_output_filename = arcpy.ValidateTableName(in_pxw_csv_path.stem)
if arcpy.GetParameterAsText(5):
    in_output_filename = arcpy.ValidateTableName(arcpy.GetParameterAsText(5))

in_use_wm_sr_for_output = arcpy.GetParameter(7)
in_should_transform_fields = arcpy.GetParameter(8)
in_transform_fields = arcpy.GetParameter(9)

in_save_temp_files = arcpy.GetParameter(10)
...
arcpy.TableToTable_conversion(in_pxw_csv, 'memory', tmp_stats_tbl)
...
arcpy.SetParameter(6, final_output_fc_path)
