...
def peek_sdmx_fields(in_csv_folder, in_field):
...
class ToolValidator(object):
    ...
    def updateMessages(self):
        ...
        if self.params[0].value and self.params[1].value:
            check = peek_sdmx_fields(self.params[0].value, self.params[1].value)
            if not check['found']:
                self.params[1].setErrorMessage(check['message'])
        
        if self.params[8].value == True:
            if self.params[0].value and self.params[9].value:
                check = peek_sdmx_fields(self.params[0].value, self.params[9].value)
                if not check['found']:
                    self.params[9].setErrorMessage(check['message'])
